...
class Chatbot:
    ...
    def get_response(self, user_input):
        ...
        # Step 1: Get system prompt and run Ollama
        ...
        try:
            ...
            if "initiate face recognition for access" in response.lower():
                ...
                if result.get("status") == "success":
                    ...
                    # Step 3: Open door after verification
                    ...
                    return f"Door opened for {user}.", True, [f"Door opened for {user}."]
                else:
                    self.verified_user = False
                    message = result.get("message", "Face recognition failed")
                    return f"Access denied: {message}", True, [f"Access denied: {message}"]

            # Step 4: After verification, handle other commands
            if self.verified_user:
                # Clean the response by removing backslashes and normalizing
                clean_response = response.replace('\\', '').lower().strip()
                
                if "lights_on_command" in clean_response:
                    self.call_api("lights_on_command")
                    return "Turning on the lights...", True, ["Turning on the lights..."]
                elif "lights_off_command" in clean_response:
                    self.call_api("lights_off_command")
                    return "Turning off the lights...", True, ["Turning off the lights..."]
                elif "fan_on_command" in clean_response:
                    self.call_api("fan_on_command")
                    return "Turning on the fan...", True, ["Turning on the fan..."]
                elif "fan_off_command" in clean_response:
                    self.call_api("fan_off_command")
                    return "Turning off the fan...", True, ["Turning off the fan..."]
                elif "humidity" in clean_response:
                    return self._handle_sensor_data("get_humidity", "humidity", "%")
                elif "weather" in clean_response or "temperature" in clean_response:
                    return self._handle_sensor_data("get_temperature", "temperature", "°C")
                elif "environment" in clean_response:
                    return self._handle_environment_data()
                elif "close_door_command" in clean_response:
                    self.call_api("close_door_command")
                    return "Closing the door...", True, ["Closing the door..."]
                
            return response.strip(), True, sentences

        except Exception as e:
            print(f"❌ Error during chatbot response: {e}")
            return "Sorry, I had an error processing your request.", False, []
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() == "exit":
            break
        response, success, sentences = chatbot.get_response(user_input)
        if success:
